...
FileManager::getDirectoryFromFileImpl(FileManager &FileMgr, StringRef Filename,
                                      bool CacheFailure) {
  if (Filename.empty())
  ...
  return FileMgr.getDirectoryRefImpl(DirName, CacheFailure);
}
...
llvm::Expected<FileEntryRef> FileManager::getFileRef(StringRef Filename,
  ...
  auto DirInfoOrErr = getDirectoryFromFileImpl(*this, Filename, CacheFailure);
...
FileEntryRef FileManager::getVirtualFileRef(StringRef Filename, off_t Size,
  ...
  auto DirInfo =
      getDirectoryFromFileImpl(*this, Filename.empty() ? "." : Filename,
                               /*CacheFailure=*/true);
  assert(DirInfo &&
