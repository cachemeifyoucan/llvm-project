...
void Debugger::Destroy(DebuggerSP &debugger_sp) {
  ...
  // BEGIN CAS
  // Release the CAS object stores that only this debugger's modules were
  // keeping alive: modules are cached in the global shared module list and
  // deliberately outlive the debugger that loaded them, so nothing else
