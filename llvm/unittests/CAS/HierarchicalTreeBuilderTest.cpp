...
TEST(HierarchicalTreeBuilderTest, Flat) {
  ...
  Optional<AnyObjectHandle> Root = expectedToOptional(Builder.create(*CAS));
  ASSERT_TRUE(Root);
...
TEST(HierarchicalTreeBuilderTest, MergeDirectoriesConflict) {
  ...
  Optional<AnyObjectHandle> Root1;
  createRoot("blob1", "/t1/d1", Root1);
  Optional<AnyObjectHandle> Root2;
  createRoot("blob2", "/t1/d1", Root2);
  Optional<AnyObjectHandle> Root3;
